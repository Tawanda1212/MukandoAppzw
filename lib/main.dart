...
class _HomeScreenState extends State<HomeScreen>{
  ...
  Widget build(BuildContext context){
    return Scaffold(
      appBar: AppBar(
        title: Column(
          ...
          children: [
            ...
            if(isOffline) Text("OFFLINE MODE", style: TextStyle(fontSize: 10, color: Colors.yellow))
          ],
        ),
        actions: [
      ...
      body: loading? Center(child: CircularProgressIndicator(color: Color(0xFF1B5E20))) : groups.isEmpty? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.groups, size: 80, color: Color(0xFF1B5E20)),
        SizedBox(height:10),
        Text("No groups yet", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        Text("Tap + to create"),
        Text("Works offline in Bindura!", style: TextStyle(color: Color(0xFF1B5E20), fontWeight: FontWeight.bold))
      ])) : ListView.builder(
        ...
        itemBuilder: (_,i){
          ...
          return Card(
            ...
            child: ListTile(
              leading: CircleAvatar(backgroundColor: Color(0xFF1B5E20), child: Text(g['name'][0].toUpperCase(), style: TextStyle(color: Colors.white))),
              title: Text(g['name'], style: TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text("${g['type']} - \$${g['monthly_amount']}"),
              trailing: Icon(Icons.arrow_forward, color: Color(0xFF1B5E20)),
